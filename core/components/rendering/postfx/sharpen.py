...
void main() {
    ...
    vec3 b = vec3(0.0);
    b += texture(u_input_tex, v_uv + vec2(-1.0, -1.0) * px).rgb;
    b += texture(u_input_tex, v_uv + vec2( 0.0, -1.0) * px).rgb;
    b += texture(u_input_tex, v_uv + vec2( 1.0, -1.0) * px).rgb;
    b += texture(u_input_tex, v_uv + vec2(-1.0,  0.0) * px).rgb;
    b += texture(u_input_tex, v_uv + vec2( 0.0,  0.0) * px).rgb;
    b += texture(u_input_tex, v_uv + vec2( 1.0,  0.0) * px).rgb;
    b += texture(u_input_tex, v_uv + vec2(-1.0,  1.0) * px).rgb;
    b += texture(u_input_tex, v_uv + vec2( 0.0,  1.0) * px).rgb;
    b += texture(u_input_tex, v_uv + vec2( 1.0,  1.0) * px).rgb;
    ...
    frag_color = vec4(c + (c - b) * u_intensity, 1.0);
}
"""


@ComponentRegistry.register
class Sharpen(GraphicsEffect):
    _allow_multiple = False
    _gizmo_icon_label = "Sh"
    render_type = "screen"
    ...
    def serialize(self) -> dict:
        d = super().serialize()
        d.update({"_intensity": self._intensity})
        return d

    @classmethod
    def deserialize(cls, data: dict) -> Sharpen:
    ...
    def _ensure_resources(self, ctx: moderngl.Context):
        ...
        self._prog = ctx.program(
            vertex_shader=SHARPEN_VERT,
            fragment_shader=SHARPEN_FRAG
        )
