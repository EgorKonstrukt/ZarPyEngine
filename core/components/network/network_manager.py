...
class NetworkManager(Component):
    ...
    def spawn_entity(self, entity: Entity, owner_id: int | None = None) -> int:
        ...
        if self.is_server:
            ...
            if ident.owner_id == self.local_id:
                ident.is_local_player = True
    ...
    def _handle_spawn(self, data: dict):
        ...
        if net_id < 0:
            ...
            if entity_data:
                net_id = int(entity_data.get("id", -1))
        ...
        if entity_data:
            ...
            ent = self.spawn_prefab(prefab_id)
            if ent is None:
                return
            entity_data = ent.serialize()
        ...
        from core.network.transport import get_transport
        ident.is_local_player = (owner_id == get_transport().local_id)
    ...
    def _handle_rpc(self, data: dict):
        ...
        for comp in ent.get_all_components():
            ...
            if callable(fn):
                try:
                    fn(args if isinstance(args, dict) else {"value": args})
                except Exception:
                    try:
                        fn()
                    except Exception:
                        pass
            alt = getattr(comp, "handle_rpc", None)
            if callable(alt):
                try:
                    alt(method, args, data.get("_sender", -1))
                except Exception:
                    pass
            handler = getattr(comp, f"rpc_{method}", None)
            if callable(handler):
                try:
    ...
    def _sync_new_peer(self, peer_id: int):
        ...
        try:
            ...
            for nid, eid in list(self._net_to_entity.items()):
                ...
                if ent is None:
                    continue
                payload = {"net_id": nid, "owner_id": ent.get_component_by_name("NetworkIdentity").owner_id if ent.get_component_by_name("NetworkIdentity") else -1, "entity": ent.serialize()}
            ...
            for msg_type, data in msgs:
                if msg_type == MessageType.NET_SPAWN:
                    ...
                    if ent:
                        ident = ent.get_component_by_name("NetworkIdentity")
                        if ident:
                            ident.apply_owner_change(int(data.get("owner_id", data.get("_sender", -1))))
                    ...
                    if ent:
                        ...
                        if isinstance(vars_data, dict):
                            for comp_key, props in vars_data.items():
                                comp = ent.get_component_by_name(comp_key)
                                if comp and isinstance(props, dict):
